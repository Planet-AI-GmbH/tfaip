...
class Conv3D(keras.layers.Layer):
    ...
    def build(self, input_shape):
        kernel_shape = [self.kernel_size[0], self.kernel_size[1], self.kernel_size[2], input_shape[-1], self.filters]
        if self.init_opt == 0:
            stddev = np.sqrt(
                2.0 / (kernel_shape[0] * kernel_shape[1] * kernel_shape[2] * kernel_shape[3] + kernel_shape[4]))
        elif self.init_opt == 1:
            stddev = 5e-2
        elif self.init_opt == 2:
            stddev = min(np.sqrt(2.0 / (kernel_shape[0] * kernel_shape[1] * kernel_shape[2] * kernel_shape[3])), 5e-2)
        else:
            stddev = 5e-2
        if self.init_opt < 0:
            kernel_initializer = keras.initializers.TruncatedNormal(0.0, -self.init_opt)
        ...
        if self.init_opt == 3:
            kernel_initializer = tf.constant_initializer(
                value=1.0 / self.kernel_size[0] / self.kernel_size[1] / self.kernel_size[2])

        self._conv_layer = keras.layers.Conv3D(
            self.filters,
            self.kernel_size,
            strides=self.strides,
            padding=self.padding,
