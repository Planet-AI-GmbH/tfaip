...
class DeviceConfigParams:
    ...
    gpu_memory: Optional[int] = field(default=None, metadata=pai_meta(
        help='Limit the per GPU memory in MB. By default the memory will grow automatically'
    ))
    soft_device_placement: bool = field(default=True, metadata=pai_meta(
        help='Set up soft device placement is enabled'
    ))
    dist_strategy: DistributionStrategy = field(default=DistributionStrategy.DEFAULT, metadata=pai_meta(
