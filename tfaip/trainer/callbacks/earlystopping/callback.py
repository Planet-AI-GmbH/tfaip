...
class EarlyStoppingCallback(Callback):
    ...
    def __init__(self, scenario: 'ScenarioBase', trainer_params: 'TrainerParams'):
        ...
        self._export_dir = os.path.join(self._params.best_model_output_dir, self._params.best_model_name) \
            if self._export_best else None
    ...
    def on_epoch_end(self, epoch, logs=None):
            ...
            if self._early_stopping_enabled:
                ...
                if (epoch + 1) % self._params.frequency == 0:
                    if self._params.mode == 'max' and self._params.current < self._params.lower_threshold:
                        logger.info(f'Not counting {self._params.current} for early stopping since lower threshold '
                        ...
                        logger.debug(f'Not counting {self._params.current} for early stopping since upper threshold '
                                     f'{self._params.upper_threshold} was not reached')
                    else:
                        logger.info(f'Early stopping progressed. (remaining iteration without improvement: '
        ...
        if self._params.mode == 'min':
            if self._params.current <= self._params.lower_threshold:
                ...
                logger.info(f'Early stopping. Reached limit value of {self._params.monitor}: '
        ...
        if self._early_stopping_enabled and self._params.n >= self._params.n_to_go:
            self.model.stop_training = True
            logger.info(f'Early stopping. Reached number of maximum iterations without improvement '
