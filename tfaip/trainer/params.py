...
class TrainerPipelines:
    train: DataPipelineParams = field(default_factory=lambda: DataPipelineParams(mode=PipelineMode.TRAINING),
                                      metadata=pai_meta(fix_dc=True, mode='flat'))
    val: DataPipelineParams = field(default_factory=lambda: DataPipelineParams(mode=PipelineMode.EVALUATION),
                                    metadata=pai_meta(fix_dc=True, mode='flat'))
...
@pai_dataclass
@dataclass
class TrainerPipelineParamsBase(Generic[TDataGeneratorTrain, TDataGeneratorVal], ABC,
                                metaclass=TrainerPipelineParamsBaseMeta):
...
@pai_dataclass
@dataclass
class TrainerPipelineParams(TrainerPipelineParamsBase[TDataGeneratorTrain, TDataGeneratorVal],
...
class TrainerParams(Generic[TScenarioParams, TTrainerPipelineParams], ABC, metaclass=TrainerParamsMeta):
    ...
    epochs: int = field(default=100, metadata=pai_meta(
        help='The number of training epochs.'
    ))
    current_epoch: int = field(default=0, metadata=pai_meta(
        help='The epoch to start with. Usually 0, but can be overwritten for resume training.'
    ))
    samples_per_epoch: int = field(default=-1, metadata=pai_meta(
        help='The number of samples (not batches!) to process per epoch. '
             'By default (-1) the size fo the training dataset.'
    ))
    scale_epoch_size: float = field(default=1, metadata=pai_meta(
    ...
    train_accum_steps: int = field(default=1, metadata=pai_meta(
    ...
    progress_bar_mode: int = field(default=1, metadata=pai_meta(
        help='Verbose level of the progress bar.'
    ))
    progbar_delta_time: float = field(default=5, metadata=pai_meta(
        help='If verbose=2 the interval after which to output the current progress'
    ))
    tf_cpp_min_log_level: int = field(default=2, metadata=pai_meta(
    ...
    val_every_n: int = field(default=1, metadata=pai_meta(
        help='Rate at which to test the model on the validation data (--data_params validation_list)'
    ))
    lav_every_n: int = field(default=0, metadata=pai_meta(
    ...
    write_checkpoints: bool = field(default=True, metadata=pai_meta(
    ...
    export_best: Optional[bool] = field(default=None, metadata=pai_meta(
        help='Continuously export the best model during testing to output_dir/best.'
    ))
    export_final: bool = field(default=True, metadata=pai_meta(
        help='Export the final model after training to output_dir/export.'
    ))
    no_train_scope: Optional[str] = field(default=None, metadata=pai_meta(
        help='Regex to match with layer names to exclude from training, i.e. the weights of these layers will not '
             'receive updates'
    ))
    ema_decay: float = field(default=0.0, metadata=pai_meta(
    ...
    random_seed: Optional[int] = field(default=None, metadata=pai_meta(
        help='Random seed for all random generators. Use this to obtain reproducible results (at least on CPU)'
    ))
    profile: bool = field(default=False, metadata=pai_meta(
    ...
    device: DeviceConfigParams = field(default_factory=DeviceConfigParams, metadata=pai_meta(
        mode='flat', help='Parameters to setup the devices such as GPUs and multi GPU training.'
    ))
    optimizer: OptimizerParams = field(default_factory=AdamOptimizer, metadata=pai_meta(
        mode='flat', help='Optimization parameters (e.g. selection of the optimizer)',
        choices=DEFAULT_OPTIMIZERS,
    ))
    learning_rate: LearningRateParams = field(default_factory=ExponentialDecayParams, metadata=pai_meta(
        mode='flat', help='Learning rate, and scheduling parameters.',
        choices=DEFAULT_SCHEDULES,
    ))
    scenario: TScenarioParams = field(default_factory=ScenarioBaseParams, metadata=pai_meta(
        mode='flat', help='Parameters of the scenario.'
    ))
    warmstart: WarmStartParams = field(default_factory=WarmStartParams, metadata=pai_meta(
        mode='flat', help='Parameters to specify parameters to load before training (e.g. warmstart or finetuning)'
    ))
    early_stopping: EarlyStoppingParams = field(default_factory=EarlyStoppingParams, metadata=pai_meta(
        mode='flat', help='Parameters to define early stopping.'
    ))

    # Pipeline setup, Training has a train and val pipeline
    gen: TTrainerPipelineParams = field(default_factory=TrainerPipelineParams, metadata=pai_meta(
        help='Parameters that setup the data generators (i.e. the input data).'
    ))

    # Additional params
    saved_checkpoint_sub_dir: str = field(default='', metadata=pai_meta(
        mode='ignore', help='Formatted checkpoint_sub_dir of the actual epoch (no formatters)'))
    checkpoint_sub_dir: str = field(default='', metadata=pai_meta(
        mode='ignore', help='`filepath` may contain placeholders such as `{epoch:02d}`'))
    checkpoint_save_freq: Union[str, int] = field(default='epoch', metadata=pai_meta(
        mode='ignore', help='or after this many epochs'))
